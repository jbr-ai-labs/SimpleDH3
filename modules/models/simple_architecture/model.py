...
class SimpleCharRNN(nn.Module):
    ...
    def __init__(self, input_size, output_size, hidden_dim, n_layers, device, bilstm=False, corrector=None):
        super(SimpleCharRNN, self).__init__()
        self.bilstm = bilstm
        self.device = device
        if self.bilstm:
            self.model_forward = SimpleCharRNNUnit(input_size, output_size, hidden_dim, n_layers, device, corrector)
            self.model_backward = SimpleCharRNNUnit(input_size, output_size, hidden_dim, n_layers, device, corrector)
        else:
            self.model = SimpleCharRNNUnit(input_size, output_size, hidden_dim, n_layers, device, corrector)
...
class SimpleCharRNNUnit(nn.Module):
    ...
    def __init__(self, input_size, output_size, hidden_dim, n_layers, device, corrector=None):
        ...
        self.corrector = corrector if corrector is not None else rescale_prediction
    ...
    def correct_coordinates(self, coordinates, finished):
        ...
        for i, c in enumerate(coordinates_l):
            ...
            if i not in finished:
                coordinates_l[i] = self.corrector(c)
            else:
                coordinates_l[i] = c
